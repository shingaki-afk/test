...
namespace ODIS.ODIS
{
    public partial class Koteietc : Form
    {
        //元データ
        ...
        public Koteietc()
        {
            ...
            if (Program.loginname == "金城　智之" || Program.loginname == "喜屋武　大祐" || Program.loginname == "太田　朋宏" || Program.loginname == "喜納　正美" || Program.loginname == "親富祖　沙耶")
            {
            }
            else
            { 
                MessageBox.Show("参照権限がありません。");
                Com.InHistory("固定保険他入力", "", "");
                return;
            }
            ...
            DataTable dtcb2 = Com.GetDB(" select distinct 項目 from dbo.k固定保険他データ取得_前月比('" + y + "','" + m + "') order by 項目 ");

            foreach (DataRow row in dtcb2.Rows)
            {
                koumoku.Items.Add(row["項目"]);
            }
            ...
            try
            {
                using (Cn = new SqlConnection(Com.SQLConstr))
                {
                    using (Cmd = Cn.CreateCommand())
                    {
                        ...
                        Cmd.CommandText = " select * from dbo.s社員検索  order by 組織CD, 現場CD";
                        sql = " select distinct 担当区分 from dbo.k固定保険他データ取得_前月比('" + y + "','" + m + "') order by 担当区分";
                        sql2 = " select distinct 項目 from dbo.k固定保険他データ取得_前月比('" + y + "','" + m + "') order by 項目 ";
                        sql3 = " select distinct 内容 from dbo.k固定保険他データ取得_前月比('" + y + "','" + m + "') order by 内容 ";
        ...
        private void GetEntryData()
        {
            ...
            //{
            //    result += " and 更新者 = '" + Program.loginname + "'";
            //}
            ...
            sql = " select * from dbo.k固定保険他データ取得_前月比('" + y + "','" + m + "') where 担当区分 like '%%' " + result + " order by 内容, 組織CD, 現場CD";
        ...
        private void button3_Click_1(object sender, EventArgs e)
        {
            ...
            //ブックをロードします
            c1XLBook1.Load(@"\\daikensrv03\21_全体共通\40_総務発信_管理\ODIS\doc\64_固定保険他申請.xlsx");
            ...
            System.Diagnostics.Process.Start(@"C:\ODIS\HOKEN\");
