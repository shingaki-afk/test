...
namespace ODIS.ODIS
{
    public partial class RinjiTeate : Form
    {
        //元データ
        ...
        public RinjiTeate()
        {
            ...
            naiyou.Items.Add("臨04_責任者手当");
            naiyou.Items.Add("臨06_遠隔監視業務");
            naiyou.Items.Add("臨07_緊急連絡先当番業務");
            naiyou.Items.Add("臨08_夜間待機手当");
            naiyou.Items.Add("臨09_応援時移動手当");
            naiyou.Items.Add("臨11_労災休業補償額");
            naiyou.Items.Add("臨12_入社祝金");
            naiyou.Items.Add("臨13_送迎手当");
            naiyou.Items.Add("臨90_下水道3種 登録手当"); 
            naiyou.Items.Add("臨99_誤計算差額");
            naiyou.Items.Add("前01_紹介謝礼金");
                ...
                using (Cn = new SqlConnection(Com.SQLConstr))
                {
                    using (Cmd = Cn.CreateCommand())
                    {
                        ...
                        if (Program.loginname == "親泊　美和子" || Program.loginname == "石井　優子" || Program.loginname == "下地　明香里" || Program.loginname == "小園　玲奈")
                        {
                            ...
                            sql = " select distinct 担当区分 from dbo.r臨時手当データ取得_前月比('" + y + "','" + m + "') where 担当区分 in ('03_施設', '04_エンジ') order by 担当区分";
                            sql2 = " select distinct 項目 from dbo.r臨時手当データ取得_前月比('" + y + "','" + m + "') where 担当区分 in ('03_施設', '04_エンジ') order by 項目 ";
                            sql3 = " select distinct 内容 from dbo.r臨時手当データ取得_前月比('" + y + "','" + m + "') where 担当区分 in ('03_施設', '04_エンジ') order by 内容 ";
                        ...
                        else if (Program.loginname == "金城　智之" || Program.loginname == "喜屋武　大祐")
                        {
                            ...
                            sql = " select distinct 担当区分 from dbo.r臨時手当データ取得_前月比('" + y + "','" + m + "') where 担当区分 like '%%' order by 担当区分";
                            sql2 = " select distinct 項目 from dbo.r臨時手当データ取得_前月比('" + y + "','" + m + "') where 担当区分 like '%%' ";
                            sql3 = " select distinct 内容 from dbo.r臨時手当データ取得_前月比('" + y + "','" + m + "') where 担当区分 like '%%' order by 内容 ";
                        }
                        else
                        {
                            Cmd.CommandText = " select * from dbo.s社員検索 where 担当区分 like '%" + Program.loginbusyo + "%' order by 組織CD, 現場CD";
                            sql = " select distinct 担当区分 from dbo.r臨時手当データ取得_前月比('" + y + "','" + m + "') where 担当区分 like '%" + Program.loginbusyo + "%' order by 担当区分";
                            sql2 = " select distinct 項目 from dbo.r臨時手当データ取得_前月比('" + y + "','" + m + "') where 担当区分 like '%" + Program.loginbusyo + "%' order by 項目 ";
                            sql3 = " select distinct 内容 from dbo.r臨時手当データ取得_前月比('" + y + "','" + m + "') where 担当区分 like '%" + Program.loginbusyo + "%' order by 内容 ";
                        }
        ...
        private void GetEntryData()
        {
            ...
            if (Program.loginname == "親泊　美和子" || Program.loginname == "石井　優子" || Program.loginname == "下地　明香里" || Program.loginname == "小園　玲奈")
            {
                sql = " select * from dbo.r臨時手当データ取得_前月比('" + y + "','" + m + "') where 担当区分 in ('03_施設', '04_エンジ') " + result + " order by 内容, 組織CD, 現場CD";
            }
            else if (Program.loginname == "金城　智之")
            {
                sql = " select * from dbo.r臨時手当データ取得_前月比('" + y + "','" + m + "') where 担当区分 like '%%' " + result + " order by 内容, 組織CD, 現場CD";
            }
            else
            {
                sql = " select * from dbo.r臨時手当データ取得_前月比('" + y + "','" + m + "') where 担当区分 like '%" + Program.loginbusyo + "%' " + result + " order by 内容, 組織CD, 現場CD";
            }
