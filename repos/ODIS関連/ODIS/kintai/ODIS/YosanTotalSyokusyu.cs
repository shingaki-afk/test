...
namespace ODIS.ODIS
{
    public partial class YosanTotalSyokusyu : Form
    {
        private string yms = "";
        private string yme = "";
        ...
        private void GetData()
        {
            ...
            string sql = " select * from dbo.y予算集計職種別_按分追加_率追加('" + cbs.SelectedItem.ToString() + "','" + cbe.SelectedItem.ToString() + "','" + zi + "','" + hikiateflg + "') order by 部門, 職種 ";
            dt = Com.GetDB(sql);
            dgvyosan.DataSource = dt;

            //表示固定
            dgvyosan.Columns[0].Frozen = true;
            dgvyosan.Columns[1].Frozen = true;
        ...
        {
            if (cbki.SelectedItem != null && cbzi.SelectedItem != null && cbs.SelectedItem != null && cbe.SelectedItem != null)
            { 
                GetData();
            }
        }
