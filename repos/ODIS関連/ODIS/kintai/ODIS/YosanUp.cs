...
namespace ODIS.ODIS
{
    public partial class YosanUp : Form
    {
        ...
        // 編集可否のキャッシュ
        private HashSet<string> _editableYm = new HashSet<string>();
        private bool _inWindow = false;

        // 管理者/最新「次」判定（トリムして厳密一致）
        private bool IsAdmin => string.Equals(Program.loginname?.Trim(), "喜屋武　大祐", StringComparison.Ordinal);
        private bool IsFinalZi => zi == zimax;

        // 年月セルの値を yyyyMM にそろえるヘルパ
        private static string NormalizeYm(object ymCellValue)
        {
            if (ymCellValue == null) return null;
            var s = ymCellValue.ToString().Trim();
            // "2025/09" or "2025-09" → "202509"
            s = s.Replace("/", "").Replace("-", "");
            // "2025年09月" 等にも一応対応
        ...
        public YosanUp()
        {
            ...
            dgvyosan.CellBeginEdit += dgvyosan_CellBeginEdit; // ★追加：編集開始時の最終ロック
        ...
        private void GetYosan(string bumon, string genba)
        {
            ...
            ApplyRowEditabilityByPolicy();   // ★追加
        }
        ...
        private void cbzi_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (dgvyosan.DataSource != null)
                ApplyRowEditabilityByPolicy();
        }
        ...
        private void cbki_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                IniSet();
                // ★IniSet内で GetDispData だけの場合があるので、表示中のグリッドにも反映
                if (dgvyosan.DataSource != null) ApplyRowEditabilityByPolicy();
            }
        }

        // ※もし関数を「PolicyId を受け取る版」で作ったなら、上の SQL を
        //   「WITH active AS(...SELECT TOP(1) PolicyId...) SELECT YM FROM dbo.ufn_YosanEditableYM_ByPolicyId((SELECT PolicyId FROM active));」
        //   に置き換えれば OK（他のコードは同じで動きます）。

        // === cbki / cbzi から期(開始年)と次を安全に取り出す ===
        private int GetSelectedKiStartYear()
        {
            // 例: "54期(2025)" → 2025
            var s = cbki.SelectedItem?.ToString() ?? "";
            var m = Regex.Match(s, @"\((\d{4})\)");
            if (m.Success) return int.Parse(m.Groups[1].Value);
            return int.Parse(yms.Substring(0, 4)); // フォールバック
        }
        private int GetSelectedZiInt()
        {
            // 例: "第3次" → 3
            var s = cbzi.SelectedItem?.ToString() ?? "";
            var m = Regex.Match(s, @"第(\d+)次");
            if (m.Success) return int.Parse(m.Groups[1].Value);
            return int.Parse(string.IsNullOrEmpty(zi) ? "0" : zi);
        }

        // === DataGridView(dgvyosan) の行を、取得した YM セットに基づき ReadOnly 切替 ===
        private void ApplyRowEditabilityByPolicy()
        {
            if (dgvyosan.DataSource == null) return;

            // ポリシーから「期間内/編集可能YMセット」を取得
            (_inWindow, _editableYm) = LoadEditableYmSetFromPolicy();

            // 期間外、もしくは最新「次」以外は、管理者以外は全ロック
            bool lockAll = !IsAdmin && (!_inWindow || !IsFinalZi);

            // グリッド全体のReadOnlyで物理ロック（これで大半のケースは止まる）
            dgvyosan.ReadOnly = lockAll;

            // 見た目と行単位のReadOnlyも反映（期間内のみ意味あり）
            foreach (DataGridViewRow row in dgvyosan.Rows)
            {
                ...
                if (!lockAll)
                {
                    var ym = NormalizeYm(row.Cells["年月"]?.Value);
                    allow = (ym != null && _editableYm.Contains(ym));
                    if (IsAdmin) allow = true;       // 管理者なら期間内は常に可
                    if (!IsAdmin && !IsFinalZi) allow = false; // 念のため
                }

                row.ReadOnly = !allow;
                row.DefaultCellStyle.BackColor = allow ? Color.White : Color.Gainsboro;
            }

            // 更新ボタンは “何か1行でも編集可” かつ “管理者or最新次” のときのみ有効
            bool anyEditable = !lockAll && dgvyosan.Rows.Cast<DataGridViewRow>()
                                .Any(r => !r.IsNewRow && !r.ReadOnly);
            button1.Enabled = (IsAdmin || IsFinalZi) && anyEditable;
        }


        private int GetSelectedKiNo()
        {
            var s = cbki.SelectedItem?.ToString() ?? "";   // 例: "54期(2025)"
            var m = System.Text.RegularExpressions.Regex.Match(s, @"^(\d+)期");
            if (m.Success) return int.Parse(m.Groups[1].Value);
            // フォールバック（必要なら y予算マスタ から期を引くなどに差し替え）
            return 0;
        }

        private static string BuildYm(int year, int month) => $"{year:D4}{month:D2}";

        private (bool inWindow, HashSet<string> ymSet) LoadEditableYmSetFromPolicy()
        {
            int kiNo = GetSelectedKiNo();     // 54 など
            int ziSel = GetSelectedZiInt();    // 第n次 → n
            int startYear = GetSelectedKiStartYear(); // 2025 など（4月始まりの年）

            const string sql = @"
SELECT TOP(1)
    [4月],[5月],[6月],[7月],[8月],[9月],[10月],[11月],[12月],[1月],[2月],[3月]
FROM dbo.YosanEditPolicy WITH (NOLOCK)
WHERE 期 = @kiNo AND 次 = @ziSel
  AND @now >= 適用開始日
  AND @now <  DATEADD(day, 1, 適用終了日)
ORDER BY PolicyId DESC;";

            var set = new HashSet<string>();

            using (var cn = new SqlConnection(Com.SQLConstr))
            using (var cmd = new SqlCommand(sql, cn))
            {
                cmd.Parameters.Add("@kiNo", SqlDbType.Int).Value = kiNo;
                cmd.Parameters.Add("@ziSel", SqlDbType.Int).Value = ziSel;
                ...
                {
                    if (!r.Read())
                        return (false, set); // 期間外（行が見つからない）→ 全月不可

                    // 4～12月は開始年、1～3月は開始年+1
                    bool m4 = r.GetBoolean(0);
                    bool m5 = r.GetBoolean(1);
                    bool m6 = r.GetBoolean(2);
                    bool m7 = r.GetBoolean(3);
                    bool m8 = r.GetBoolean(4);
                    bool m9 = r.GetBoolean(5);
                    bool m10 = r.GetBoolean(6);
                    bool m11 = r.GetBoolean(7);
                    bool m12 = r.GetBoolean(8);
                    bool m1 = r.GetBoolean(9);
                    bool m2 = r.GetBoolean(10);
                    bool m3 = r.GetBoolean(11);

                    if (m4) set.Add(BuildYm(startYear, 4));
                    if (m5) set.Add(BuildYm(startYear, 5));
                    if (m6) set.Add(BuildYm(startYear, 6));
                    if (m7) set.Add(BuildYm(startYear, 7));
                    if (m8) set.Add(BuildYm(startYear, 8));
                    if (m9) set.Add(BuildYm(startYear, 9));
                    if (m10) set.Add(BuildYm(startYear, 10));
                    if (m11) set.Add(BuildYm(startYear, 11));
                    if (m12) set.Add(BuildYm(startYear, 12));
                    if (m1) set.Add(BuildYm(startYear + 1, 1));
                    if (m2) set.Add(BuildYm(startYear + 1, 2));
                    if (m3) set.Add(BuildYm(startYear + 1, 3));
                }
            }
        ...
        private void dgvyosan_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            if (IsAdmin) return; // 管理者は期間内はOK
                                 // 期間外 or 最新「次」以外 → 常に編集不可
            if (!_inWindow || !IsFinalZi) { e.Cancel = true; return; }

            // 期間内でも編集可能月以外は不可
            var ym = NormalizeYm(dgvyosan.Rows[e.RowIndex].Cells["年月"]?.Value);
            if (ym == null || !_editableYm.Contains(ym))
            {
                e.Cancel = true;
                return;
            }
