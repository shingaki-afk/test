...
namespace ODIS.ODIS
{
    public partial class YosanTsukibetsu : Form
    {
        //YM
        private string maey = "";
        private string atoy = "";
        ...
        private void SetTiku()
        {
            ...
            string sql = "select distinct 担当区分 from dbo.Yosankanri where 年月 between '" + maey + "04' and '" + atoy + "03' and 次 = '2' order by 担当区分 ";
            dt = Com.GetDB(sql);
        ...
        private void SetBumon()
        {
            ...
            //TODO 次がうめこみ！！
            DataTable dt = new DataTable();
            string sql = "select distinct 職種 from dbo.Yosankanri where 年月 between '" + maey + "04' and '" + atoy + "03' and 次 = '2'";
        ...
        private void SetGenba()
        {
            ...
            //TODO 次がうめこみ！！
            //売上
            string sql = "select distinct 現場CD,現場名 from dbo.Yosankanri　where 年月 between '" + maey + "04' and '" + atoy + "03' and 次 = '2'";
        ...
        private void GetData()
        {
            ...
            if (cbsyouyo.Checked)
            {
                ...
                if (!cbgoukei.Checked) sql += "sum(実績_固定売上) as 実績_固定売上, sum(実績_臨時売上) as 実績_臨時売上, ";
                sql += "sum(実績_売上) as 実績_売上,";
                if (!cbgoukei.Checked) sql += "sum(実績_人件費) as 実績_人件費,sum(実績_諸経費) as 実績_諸経費,  ";
                sql += "sum(実績_経費) as 実績_経費,  sum(実績_現場利益) as 実績_現場利益, sum(実績_現場計数) as 実績_現場計数, ";
                if (!cbgoukei.Checked) sql += "sum(実績_管理人件費) as 実績_管理人件費,sum(実績_管理諸経費) as 実績_管理諸経費, ";
                sql += "sum(実績_管理経費) as 実績_管理経費, sum(実績_管理計数) as 実績_管理計数, sum(実績_部門利益) as 実績_部門利益, sum(実績_部門計数) as 実績_部門計数 ";
                sql += "from(select 年月, ";
                ...
                sql += "0 as 実績_固定売上, 0 as 実績_臨時売上, 0 as 実績_売上, 0 as 実績_人件費,0 as 実績_諸経費, 0 as 実績_経費, 0 as 実績_現場利益, 0 as 実績_現場計数,   ";
                sql += "0 as 実績_管理人件費,0 as 実績_管理諸経費, 0 as 実績_管理経費, 0 as 実績_管理計数, 0 as 実績_部門利益, 0 as 実績_部門計数 ";
                sql += "from dbo.yosankanri a left join dbo.担当テーブル b on a.部門CD = b.組織CD and a.現場CD = b.現場CD where 年月 between '202404' and '202503' and 次 = '6' ";
                sql += GetTSG();
                sql += " group by 年月 ";
                ...
                sql += "sum(固定売上) as 実績_固定売上, sum(臨時売上) as 実績_臨時売上, sum(固定売上) + sum(臨時売上) as 実績_売上,   ";
                sql += "sum(人件費) as 実績_人件費,sum(諸経費) as 実績_諸経費,  ";
                sql += "sum(人件費) + sum(諸経費) as 実績_経費,  sum(固定売上) + sum(臨時売上) - sum(人件費) - sum(諸経費) as 実績_現場利益,   ";
                sql += "case when sum(固定売上) +sum(臨時売上) = 0 then 0 else (sum(人件費) + sum(諸経費)) / (sum(固定売上) + sum(臨時売上)) * 100 end as 実績_現場計数,   ";
                sql += "sum(管理人件費) as 実績_管理人件費,sum(管理諸経費) as 実績_管理諸経費, sum(管理人件費) + sum(管理諸経費) as 実績_管理経費,   ";
                sql += "case when sum(固定売上) +sum(臨時売上) = 0 then 0 else (sum(管理人件費) + sum(管理諸経費)) / (sum(固定売上) + sum(臨時売上)) * 100 end as 実績_管理計数,   ";
                sql += "sum(固定売上) + sum(臨時売上) - sum(人件費) - sum(諸経費) - sum(管理人件費) - sum(管理諸経費) as 実績_部門利益,   ";
                sql += "case when sum(固定売上) +sum(臨時売上) = 0 then 0 else (sum(人件費) + sum(諸経費) + sum(管理人件費) + sum(管理諸経費)) / (sum(固定売上) + sum(臨時売上)) * 100 end as 実績_部門計数 ";
            ...
            else
            {
                ...
                if (!cbgoukei.Checked)
                {
                    sql += "sum(予算_固定売上) as 固定売上_予算, sum(実績_固定売上) as 固定売上_実績, sum(予算_固定売上) - sum(実績_固定売上) as 固定売上_差額, " +
                           "sum(予算_臨時売上) as 臨時売上_予算, sum(実績_臨時売上) as 臨時売上_実績, sum(予算_固定売上) - sum(実績_固定売上) as 固定売上_差額, ";
                }
                sql += "sum(予算_売上) as 売上_予算, sum(実績_売上) as 売上_実績, sum(予算_売上) - sum(実績_売上) as 売上_差額, ";

                if (!cbgoukei.Checked)
                {
                    sql += "sum(予算_人件費) as 人件費_予算, sum(実績_人件費) as 人件費_実績, sum(予算_人件費) - sum(実績_人件費) as 人件費_差額, " +
                           "sum(予算_賞与) as 賞与_予算, sum(実績_賞与) as 賞与_実績, sum(予算_賞与) - sum(実績_賞与) as 賞与_差額, " +
                           "sum(予算_退職金) as 退職金_予算, sum(実績_退職金) as 退職金_実績, sum(予算_退職金) - sum(実績_退職金) as 退職金_差額, " +
                           "sum(予算_諸経費) as 諸経費_予算, sum(実績_諸経費) as 諸経費_実績, sum(予算_諸経費) - sum(実績_諸経費) as 諸経費_差額, ";
                }
                sql += "sum(予算_経費) as 経費_予算, sum(実績_経費) as 経費_実績, sum(予算_経費) - sum(実績_経費) as 経費_差額, " +
                       "sum(予算_現場利益) as 現場利益_予算, sum(実績_現場利益) as 現場利益_実績, sum(予算_現場利益) - sum(実績_現場利益) as 現場利益_差額, " +
                       "sum(予算_現場計数) as 現場計数_予算, sum(実績_現場計数) as 現場計数_実績, sum(予算_現場計数) - sum(実績_現場計数) as 現場計数_差額,   ";

                if (!cbgoukei.Checked)
                {
                    sql += "sum(予算_管理人件費) as 管理人件費_予算, sum(実績_管理人件費) as 管理人件費_実績, sum(予算_管理人件費) - sum(実績_管理人件費) as  管理人件費_差額, " +
                          " sum(予算_管理賞与) as 管理賞与_予算, sum(予算_管理退職金) as 管理退職金_予算, sum(実績_管理賞与) as 管理賞与_実績, sum(実績_管理退職金) as 管理退職金_実績, sum(予算_管理賞与) - sum(実績_管理賞与) as 管理賞与_差額, sum(予算_管理退職金) - sum(実績_管理退職金) as 管理退職金_差額, " +
                          " sum(予算_管理諸経費) as管理諸経費_予算, sum(実績_管理諸経費) as 管理諸経費_実績, sum(予算_管理諸経費) - sum(実績_管理諸経費) as 管理諸経費_差額, ";
                }
                sql += "sum(予算_管理経費) as 管理経費_予算, sum(実績_管理経費) as 管理経費_実績, sum(予算_管理経費) - sum(実績_管理経費) as 管理経費_差額, " +
                       "sum(予算_管理計数) as 管理計数_予算, sum(実績_管理計数) as 管理計数_実績, sum(予算_管理計数) - sum(実績_管理計数) as 管理計数_差額, " +
                       "sum(予算_部門利益) as 部門利益_予算, sum(実績_部門利益) as 部門利益_実績, sum(予算_部門利益) - sum(実績_部門利益) as 部門利益_差額, " +
                       "sum(予算_部門計数) as 部門計数_予算, sum(実績_部門計数) as 部門計数_実績, sum(予算_部門計数) - sum(実績_部門計数) as 部門計数_差額 ";

                sql += "from(select 年月, ";

                //予算
                sql += "sum(固定売上) as 予算_固定売上, sum(臨時売上) as 予算_臨時売上, sum(固定売上) + sum(臨時売上) as 予算_売上, ";
                sql += "sum(人件費) as 予算_人件費, sum(賞与) as 予算_賞与, sum(退職金) as 予算_退職金, sum(諸経費) as 予算_諸経費, ";
                sql += "sum(人件費) + sum(賞与) + sum(退職金) + sum(諸経費) as 予算_経費, sum(固定売上) + sum(臨時売上) - sum(人件費) - sum(賞与) - sum(退職金) - sum(諸経費) as 予算_現場利益, ";
                sql += "case when sum(固定売上) + sum(臨時売上) = 0 then 0 else (sum(人件費) + sum(賞与) + sum(退職金) + sum(諸経費)) / (sum(固定売上) + sum(臨時売上)) * 100 end as 予算_現場計数,   ";
                sql += "sum(管理人件費) as 予算_管理人件費,sum(管理賞与) as 予算_管理賞与,sum(管理退職金) as 予算_管理退職金,sum(管理諸経費) as 予算_管理諸経費, sum(管理人件費) + sum(管理賞与) + sum(管理退職金) + sum(管理諸経費) as 予算_管理経費,   ";
                sql += "case when sum(固定売上) +sum(臨時売上) = 0 then 0 else (sum(管理人件費) + sum(管理賞与) + sum(管理退職金) + sum(管理諸経費)) / (sum(固定売上) + sum(臨時売上)) * 100 end as 予算_管理計数,   ";
                sql += "sum(固定売上) + sum(臨時売上) - sum(人件費) - sum(賞与) - sum(退職金) - sum(諸経費) - sum(管理人件費) - sum(管理賞与) - sum(管理退職金) - sum(管理諸経費) as 予算_部門利益,   ";
                sql += "case when sum(固定売上) +sum(臨時売上) = 0 then 0 else (sum(人件費) + sum(賞与) + sum(退職金) + sum(諸経費) + sum(管理人件費) + sum(管理賞与) + sum(管理退職金) + sum(管理諸経費)) / (sum(固定売上) + sum(臨時売上)) * 100 end as 予算_部門計数,  ";

                sql += "0 as 実績_固定売上, 0 as 実績_臨時売上, 0 as 実績_売上, 0 as 実績_人件費, 0 as 実績_賞与, 0 as 実績_退職金,0 as 実績_諸経費, 0 as 実績_経費, 0 as 実績_現場利益, 0 as 実績_現場計数,   ";
                sql += "0 as 実績_管理人件費,0 as 実績_管理賞与,0 as 実績_管理退職金,0 as 実績_管理諸経費, 0 as 実績_管理経費, 0 as 実績_管理計数, 0 as 実績_部門利益, 0 as 実績_部門計数 ";
                sql += "from dbo.yosankanri a left join dbo.担当テーブル b on a.部門CD = b.組織CD and a.現場CD = b.現場CD where 年月 between '202404' and '202503' and 次 = '6' ";
                sql += GetTSG();
                sql += " group by 年月 ";
                ...
                //実績
                sql += "select 年月, ";
                sql += "0 as 予算_固定売上, 0 as 予算_臨時売上, 0 as 予算_売上, 0 as 予算_人件費, 0 as 予算_賞与, 0 as 予算_退職金,0 as 予算_諸経費, 0 as 予算_経費,  0 as 予算_現場利益, 0 as 予算_現場計数,   ";
                sql += "0 as 予算_管理人件費,0 as 予算_管理賞与,0 as 予算_管理退職金,0 as 予算_管理諸経費, 0 as 予算_管理経費, 0 as 予算_管理計数, 0 as 予算_部門利益, 0 as 予算_部門計数,  ";

                sql += "sum(固定売上) as 実績_固定売上, sum(臨時売上) as 実績_臨時売上, sum(固定売上) + sum(臨時売上) as 実績_売上,   ";
                sql += "sum(人件費) as 実績_人件費, sum(賞与) as 実績_賞与, sum(退職金) as 実績_退職金,sum(諸経費) as 実績_諸経費,  ";
                sql += "sum(人件費) + sum(賞与) + sum(退職金) + sum(諸経費) as 実績_経費,  sum(固定売上) + sum(臨時売上) - sum(人件費) - sum(賞与) - sum(退職金) - sum(諸経費) as 実績_現場利益,   ";
                sql += "case when sum(固定売上) +sum(臨時売上) = 0 then 0 else (sum(人件費) + sum(賞与) + sum(退職金) + sum(諸経費)) / (sum(固定売上) + sum(臨時売上)) * 100 end as 実績_現場計数,   ";
                sql += "sum(管理人件費) as 実績_管理人件費,sum(管理賞与) as 実績_管理賞与,sum(管理退職金) as 実績_管理退職金,sum(管理諸経費) as 実績_管理諸経費, sum(管理人件費) + sum(管理賞与) + sum(管理退職金) + sum(管理諸経費) as 実績_管理経費,   ";
                sql += "case when sum(固定売上) +sum(臨時売上) = 0 then 0 else (sum(管理人件費) + sum(管理賞与) + sum(管理諸経費)) / (sum(固定売上) + sum(臨時売上)) * 100 end as 実績_管理計数,   ";
                sql += "sum(固定売上) + sum(臨時売上) - sum(人件費) - sum(賞与) - sum(退職金) - sum(諸経費) - sum(管理人件費) - sum(管理賞与) - sum(管理退職金) - sum(管理諸経費) as 実績_部門利益,   ";
                sql += "case when sum(固定売上) +sum(臨時売上) = 0 then 0 else (sum(人件費) + sum(賞与) + sum(退職金) + sum(諸経費) + sum(管理人件費) + sum(管理賞与) + sum(管理退職金) + sum(管理諸経費)) / (sum(固定売上) + sum(臨時売上)) * 100 end as 実績_部門計数 ";
                sql += "from dbo.kanrikeisuu a left join dbo.担当テーブル b on a.部門CD = b.組織CD and a.現場CD = b.現場CD where 年月 between '202404' and '202503' ";
                sql += GetTSG();
                sql += " group by 年月 ";
                sql += ") temp group by 年月 ";
            ...
            if (cbgoukei.Checked)
            {
                ...
                {
                    //現場計数
                    gendt.Rows[9]["年間"] = gendt.Rows[9]["月平均"] = Convert.ToDecimal(gendt.Rows[3]["年間"]) / Convert.ToDecimal(gendt.Rows[0]["年間"]) * 100;

                    //管理計数
                    gendt.Rows[15]["年間"] = gendt.Rows[15]["月平均"] = Convert.ToDecimal(gendt.Rows[12]["年間"]) / Convert.ToDecimal(gendt.Rows[0]["年間"]) * 100;

                    //部門計数
                    gendt.Rows[21]["年間"] = gendt.Rows[21]["月平均"] = (Convert.ToDecimal(gendt.Rows[3]["年間"]) + Convert.ToDecimal(gendt.Rows[12]["年間"])) / Convert.ToDecimal(gendt.Rows[0]["年間"]) * 100;
                }

                //前期
                if (Convert.ToDecimal(gendt.Rows[1]["年間"]) > 0)
                {
                    //現場計数
                    gendt.Rows[10]["年間"] = gendt.Rows[10]["月平均"] = Convert.ToDecimal(gendt.Rows[4]["年間"]) / Convert.ToDecimal(gendt.Rows[1]["年間"]) * 100;

                    //管理計数
                    gendt.Rows[16]["年間"] = gendt.Rows[16]["月平均"] = Convert.ToDecimal(gendt.Rows[13]["年間"]) / Convert.ToDecimal(gendt.Rows[1]["年間"]) * 100;

                    //部門計数
                    gendt.Rows[22]["年間"] = gendt.Rows[22]["月平均"] = (Convert.ToDecimal(gendt.Rows[4]["年間"]) + Convert.ToDecimal(gendt.Rows[13]["年間"])) / Convert.ToDecimal(gendt.Rows[1]["年間"]) * 100;
                }

                //差額
                gendt.Rows[11]["年間"] = gendt.Rows[11]["月平均"] = Convert.ToDecimal(gendt.Rows[9]["年間"]) - Convert.ToDecimal(gendt.Rows[10]["年間"]);
                gendt.Rows[17]["年間"] = gendt.Rows[17]["月平均"] = Convert.ToDecimal(gendt.Rows[15]["年間"]) - Convert.ToDecimal(gendt.Rows[16]["年間"]);
                gendt.Rows[23]["年間"] = gendt.Rows[23]["月平均"] = Convert.ToDecimal(gendt.Rows[21]["年間"]) - Convert.ToDecimal(gendt.Rows[22]["年間"]);

                //TODO 差額、おそらく↓これ必要
                //if (Convert.ToDecimal(gendt.Rows[15]["年間"]) > 0 && Convert.ToDecimal(gendt.Rows[16]["年間"]) > 0)
                //{
            ...
            for (int i = 1; i < 15; i++)
            {
                ...
                dataGridView1.Columns[i].Width = 68;
        ...
        private void dataGridView1_SelectionChanged(object sender, EventArgs e)
        {
            ...
            if (Regex.IsMatch(row, "予算"))
            {
                if (Regex.IsMatch(row, "売上"))
                { 
                    sql += " select 年月, 次, a.部門CD, a.現場CD, a.部門名, a.現場名, 固定売上, 臨時売上 from dbo.yosankanri a left join dbo.担当テーブル b on a.部門CD = b.組織CD and a.現場CD = b.現場CD where 次 = '2' ";
                }
                else
                {
                    //TODO
                    sql += " select * from dbo.yosankanri a left join dbo.担当テーブル b on a.部門CD = b.組織CD and a.現場CD = b.現場CD where 次 = '2' ";
                }
            }
            else if (Regex.IsMatch(row, "前期"))
            {
                //TODO
                sql += " select * from dbo.kanrikeisuu a left join dbo.担当テーブル b on a.部門CD = b.組織CD and a.現場CD = b.現場CD where 次 = '2' ";
            }
            ...
            if (col == "年間")
            {
                sql += " and 年月 between '" + maey + "0401' and '" + atoy + "0331'";
            }
            else
            {
                if (col == "01月" || col == "02月" || col == "03月")
                {
                    sql += " and 年月 like '" + atoy + col.Replace("月", "") + "%'";
                }
                else
                {
                    sql += " and 年月 like '" + maey + col.Replace("月", "") + "%'";
                }
            }
    ...
    private void blue_ValueChanged(object sender, EventArgs e)
    {
        ...
        private DataTable MonthDataTable(DataTable dt)
        {
            ...
            gendt.Columns.Add("04月", typeof(decimal));
            gendt.Columns.Add("05月", typeof(decimal));
            gendt.Columns.Add("06月", typeof(decimal));
            gendt.Columns.Add("07月", typeof(decimal));
            gendt.Columns.Add("08月", typeof(decimal));
            gendt.Columns.Add("09月", typeof(decimal));
            gendt.Columns.Add("10月", typeof(decimal));
            gendt.Columns.Add("11月", typeof(decimal));
            gendt.Columns.Add("12月", typeof(decimal));
            gendt.Columns.Add("01月", typeof(decimal));
            gendt.Columns.Add("02月", typeof(decimal));
            gendt.Columns.Add("03月", typeof(decimal));
            ...
            foreach (DataRow row in wkdt.Rows)
            {
                ...
                nr["項目"] = row["年月"];
                if (row.Table.Columns.Contains("202404")) { nr["04月"] = row["202404"]; sum += Convert.ToDecimal(row["202404"]); }
                if (row.Table.Columns.Contains("202405")) { nr["05月"] = row["202405"]; sum += Convert.ToDecimal(row["202405"]); }
                if (row.Table.Columns.Contains("202406")) { nr["06月"] = row["202406"]; sum += Convert.ToDecimal(row["202406"]); }
                if (row.Table.Columns.Contains("202407")) { nr["07月"] = row["202407"]; sum += Convert.ToDecimal(row["202407"]); }
                if (row.Table.Columns.Contains("202408")) { nr["08月"] = row["202408"]; sum += Convert.ToDecimal(row["202408"]); }
                if (row.Table.Columns.Contains("202409")) { nr["09月"] = row["202409"]; sum += Convert.ToDecimal(row["202409"]); }
                if (row.Table.Columns.Contains("202410")) { nr["10月"] = row["202410"]; sum += Convert.ToDecimal(row["202410"]); }
                if (row.Table.Columns.Contains("202411")) { nr["11月"] = row["202411"]; sum += Convert.ToDecimal(row["202411"]); }
                if (row.Table.Columns.Contains("202412")) { nr["12月"] = row["202412"]; sum += Convert.ToDecimal(row["202412"]); }
                if (row.Table.Columns.Contains("202501")) { nr["01月"] = row["202501"]; sum += Convert.ToDecimal(row["202501"]); }
                if (row.Table.Columns.Contains("202502")) { nr["02月"] = row["202502"]; sum += Convert.ToDecimal(row["202502"]); }
                if (row.Table.Columns.Contains("202503")) { nr["03月"] = row["202503"]; sum += Convert.ToDecimal(row["202503"]); }
        ...
        {
            if (e.Value != null && e.ColumnIndex == 0 && Regex.IsMatch(e.Value.ToString(), "差額", RegexOptions.Singleline) && e.RowIndex % 2 == 0)
            {
                dataGridView1.Rows[e.RowIndex].DefaultCellStyle.BackColor = Color.PowderBlue;
            }
            else if (e.Value != null && e.ColumnIndex == 0 && Regex.IsMatch(e.Value.ToString(), "差額", RegexOptions.Singleline) && e.RowIndex % 2 != 0)
            {
                dataGridView1.Rows[e.RowIndex].DefaultCellStyle.BackColor = Color.Thistle;
            }
        }
