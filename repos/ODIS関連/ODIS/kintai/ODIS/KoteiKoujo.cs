...
namespace ODIS.ODIS
{
    public partial class KoteiKoujo : Form
    {
        //元データ
        ...
        public KoteiKoujo()
        {
            ...
            if (Program.loginname == "金城　智之" || Program.loginname == "喜屋武　大祐" || Program.loginname == "太田　朋宏")
            {
                naiyou.Items.Add("固00_持株貸付返済");//総務
                naiyou.Items.Add("固01_浦添駐車場代");
                naiyou.Items.Add("固02_借入金/立替返済");
                naiyou.Items.Add("固03_中北部駐車場代");
                naiyou.Items.Add("固04_サンエー駐車場代");//総務
                naiyou.Items.Add("固05_新都心駐車代");
                naiyou.Items.Add("固06_ダブルツリー駐車場代");
                naiyou.Items.Add("固07_二十日会賄費");//総務
                naiyou.Items.Add("固08_寮固定費");
                naiyou.Items.Add("固09_社宅費");
                naiyou.Items.Add("固10_家賃代超過分");
                naiyou.Items.Add("その他※備考に内容記載下さい");
            ...
            else
            {
                ...
                //TODO 追加される場合はこちらも変更が必要！！
                soumuonly = "('固00_持株貸付返済','固04_サンエー駐車場代','固07_二十日会賄費')";
            }
                ...
                using (Cn = new SqlConnection(Com.SQLConstr))
                {
                    using (Cmd = Cn.CreateCommand())
                    {
                        ...
                        if (Program.loginname == "親泊　美和子" || Program.loginname == "石井　優子" || Program.loginname == "下地　明香里" || Program.loginname == "小園　玲奈")
                        {
                            ...
                            sql = " select distinct 担当区分 from dbo.k固定控除データ取得_前月比('" + y + "','" + m + "') where 担当区分 in ('03_施設', '04_エンジ') and 内容 not in " + soumuonly + " order by 担当区分";
                            sql2 = " select distinct 項目 from dbo.k固定控除データ取得_前月比('" + y + "','" + m + "') where 担当区分 in ('03_施設', '04_エンジ') and 内容 not in " + soumuonly + " order by 項目 ";
                            sql3 = " select distinct 内容 from dbo.k固定控除データ取得_前月比('" + y + "','" + m + "') where 担当区分 in ('03_施設', '04_エンジ') and 内容 not in " + soumuonly + " order by 内容 ";
                        ...
                        else if (Program.loginname == "金城　智之" || Program.loginname == "喜屋武　大祐")
                        {
                            ...
                            sql = " select distinct 担当区分 from dbo.k固定控除データ取得_前月比('" + y + "','" + m + "') where 担当区分 like '%%' order by 担当区分";
                            sql2 = " select distinct 項目 from dbo.k固定控除データ取得_前月比('" + y + "','" + m + "') where 担当区分 like '%%' ";
                            sql3 = " select distinct 内容 from dbo.k固定控除データ取得_前月比('" + y + "','" + m + "') where 担当区分 like '%%' order by 内容 ";
                        }
                        else
                        {
                            Cmd.CommandText = " select * from dbo.s社員検索 where 担当区分 like '%" + Program.loginbusyo + "%' order by 組織CD, 現場CD";
                            sql = " select distinct 担当区分 from dbo.k固定控除データ取得_前月比('" + y + "','" + m + "') where 担当区分 like '%" + Program.loginbusyo + "%' and 内容 not in " + soumuonly + " order by 担当区分";
                            sql2 = " select distinct 項目 from dbo.k固定控除データ取得_前月比('" + y + "','" + m + "') where 担当区分 like '%" + Program.loginbusyo + "%' and 内容 not in " + soumuonly + " order by 項目 ";
                            sql3 = " select distinct 内容 from dbo.k固定控除データ取得_前月比('" + y + "','" + m + "') where 担当区分 like '%" + Program.loginbusyo + "%' and 内容 not in " + soumuonly + " order by 内容 ";
                        }
        ...
        private void GetEntryData()
        {
            ...
            if (Program.loginname == "親泊　美和子" || Program.loginname == "石井　優子" || Program.loginname == "下地　明香里" || Program.loginname == "小園　玲奈")
            {
                sql = " select * from dbo.k固定控除データ取得_前月比('" + y + "','" + m + "') where 担当区分 in ('03_施設', '04_エンジ') and 内容 not in " + soumuonly + " " + result + " order by 内容, 組織CD, 現場CD";
            }
            else if (Program.loginname == "金城　智之")
            {
                sql = " select * from dbo.k固定控除データ取得_前月比('" + y + "','" + m + "') where 担当区分 like '%%' and 内容 not in " + soumuonly + " " + result + " order by 内容, 組織CD, 現場CD";
            }
            else
            {
                sql = " select * from dbo.k固定控除データ取得_前月比('" + y + "','" + m + "') where 担当区分 like '%" + Program.loginbusyo + "%' and 内容 not in " + soumuonly + " " + result + " order by 内容, 組織CD, 現場CD";
            }
        ...
        private void button3_Click_1(object sender, EventArgs e)
        {
            ...
            //ブックをロードします
            c1XLBook1.Load(@"\\daikensrv03\21_全体共通\40_総務発信_管理\ODIS\doc\63_固定控除申請.xlsx");
