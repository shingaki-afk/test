        ...
        public SyoteiDays()
        {
            ...
            using (Cn = new SqlConnection(Common.constr))
            {
                ...
                using (Cmd = Cn.CreateCommand())
                {
                    ...
                    Cmd.CommandText = "select * from dbo.c所定労働テーブル where 年月 = (select LEFT(convert(varchar, 対象開始日, 112), 6) from dbo.[001_対象年月マスタ])";
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                //donichiDay = (int)row["土日祝休"];
                mtotalDay = (int)row["所定"] + (int)row["休日"];
                //kizyun5 = (int)row["土日祝休"];
                //kizyun4 = (int)row["土日祝休"] + 4;
                //kizyun3 = (int)row["土日祝休"] + 8;
                //kizyun2 = (int)row["土日祝休"] + 12;
                //kizyun1 = (int)row["土日祝休"] + 16;
